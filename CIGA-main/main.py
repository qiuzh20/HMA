...
from models.ciga import GNNERM, CIGA, GNNPooling, GNNERM_LGMA
...
def main():
    ...
    # LGMA parameters
    parser.add_argument('--lgma', type = str2bool, default = False)
    parser.add_argument('--label_hidden', type=int, default=16)
    # attention config
    ...
    parser.add_argument('--logits_resource', default='label_emb', type=str, choices=['label_emb', 'feature', 'all'])
    ...
    exp_dir = os.path.join('./logs/', args.dataset)
    make_dir(exp_dir)
    experiment_name = f'{args.bias}_{args.ginv_opt}_erm{args.erm}_dir{args.dir}_coes{args.contrast}-spu_coe{args.spu_coe}_{datetime_now[4::]}'
    ...
    for seed in args.seed:
        ...
        if args.erm:
            if args.lgma:
                model = GNNERM_LGMA(input_dim=input_dim,
                            edge_dim=edge_dim,
                            args=args,
                            out_dim=num_classes,
                            gnn_type=args.model,
                ...
                if args.dir > 0:
                    ...
                    if args.spu_coe > 0 and not args.erm:
                        ...
                        if args.contrast_rep.lower() == "feat":
                            ...
                            if args.lgma:
                                causal_pred, causal_rep = model(graph, label=graph.y[is_labeled], return_data="rep")
                            else:
