...
class MAB(nn.Module):
    ...
    def __init__(
        ...
        * `F_A` is dimension of the embedding at input
        ...
        if self.topk is not None:
            self.k_mask =  torch.zeros((num_heads, 128, 128), device=c.device)
            # self.__setattr__('k_mask', self.k_mask)
    ...
    def forward(self, X, Y, mask=None):
        ...
        if self.null_KV:
            attatch_k_zero = torch.zeros((K.shape[0], 1, K.shape[-1]), device=K.device)
        ...
        if self.topk is not None and self.topk < A.shape[1]:
            # debug
            # k = torch.topk(A, dim = 2, k = self.topk)
            k = torch.topk(A, dim = 1, k = self.topk)
            if A.shape[-1] != self.k_mask.shape[-1]:
                self.k_mask = torch.zeros(A.size()).to(A.device)
            k_mask = self.k_mask.scatter(2, k.indices, 1)
            # avoid IO to GPU
            # k_mask = torch.zeros(A.size()).to(A.device)
            # k_mask.scatter_(2, k.indices, 1)
            A = A * k_mask
            # self.k_mask *= 0
