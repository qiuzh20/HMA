...
from models.HMA.model import HMA
...
class GNNERM_LGMA(nn.Module):
    ...
    def __init__(self,
                 ...
                 JK="last",
                 graph_pooling="mean"):
        super(GNNERM_LGMA, self).__init__()
        device = torch.device("cuda:" + str(args.device)) if torch.cuda.is_available() else torch.device("cpu")


        self.encoder = GNN(gnn_type=gnn_type,
                              input_dim=input_dim,
                              num_class=out_dim,
                              num_layer=num_layers,
                              emb_dim=emb_dim,
                              ...
                              JK=JK,
                              edge_dim=edge_dim)
        attention_config = {"model_num_heads":args.num_heads, 
                            'viz_att_maps': False, 
                            'model_hidden_dropout_prob': 0.1, 
                            'model_layer_norm_eps': 1e-12,  
                            'null_KV':args.null_attention,
                            'topk':args.topk, 'device':device}
        memory_config = {"num_global_memory":args.global_memory_number,
                        "local_mem_size":args.local_mem_size}
        model_config = {'label_hidden':args.label_hidden,
                        "dim_in":emb_dim, 
                        "dim_out":emb_dim,
                        "sep_label":False,
                        "encoder_momentum":args.encoder_momentum,
                        'abd_augmentation':args.abd_augmentation,
                        "logits_resource":args.logits_resource}
        
        self.classifier = HMA(encoder_model=self.encoder, config=[attention_config, memory_config, model_config], num_classes=out_dim)
        
    def forward(self, batch, label=None, return_data="pred"):
        causal_pred, causal_rep = self.classifier(batch, label, get_rep=True)
        if return_data.lower() == "pred":
            return causal_pred
        elif return_data.lower() == "rep":
            return causal_pred, causal_rep
