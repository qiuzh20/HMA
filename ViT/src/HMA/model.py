...
import numpy as np
from src.HMA.MAB_modules import MHSA
import copy
...
class HMA(torch.nn.Module):
    def __init__(self, encoder_model:torch.nn.Module, config, num_classes):
        ...
        self.gaussain_mem = self.memory_config.gaussain_mem
        ...
        if self.local_mem_size and not self.model_config.aba_ablation:
        ...
        elif self.model_config.aba_ablation:
            print("Aba ablation is on")
            self.local_read_block = MHSA(self.dim_in, self.dim_in, self.dim_out, self.attention_config)
        ...
        if self.abd_augmentation and not self.model_config.abd_ablation:
            ...
            if self.num_global_memory:
                if self.gaussain_mem:
                    print("using gaussain memory")
                    self.global_memories = torch.nn.Parameter(torch.randn(self.num_global_memory * (num_classes+self.shared_mem+(self.num_classes==1)), self.dim_in)*0.02, requires_grad=True)
                else:
                    print("using zero memory")
                    self.global_memories = torch.nn.Parameter(torch.zeros(self.num_global_memory * (num_classes+self.shared_mem+(self.num_classes==1)), self.dim_in), requires_grad=True)
        ...
        elif self.model_config.abd_ablation:
            print("Abd ablation is on")
            self.ABD_block = MHSA(self.dim_in, self.dim_in, self.dim_out, self.attention_config)
    ...
    def register_args(parser):
        ...
        """
        parser.add_argument('--label_hidden', type=int, default=64)
        # attention config
        parser.add_argument('--topk', type = int, default = None) # may be important for cifa100
        parser.add_argument('--norm_after_topk', type = str2bool, default = False)
        parser.add_argument('--num_heads', type=int, default=4)
        parser.add_argument('--null_KV', type = str2bool, default = False)
        parser.add_argument('--softmax_temp', type=float, default=1.0)
        # local memory config
        parser.add_argument('--local_mem_size', type=int, default=256)
        parser.add_argument('--encoder_momentum', type=float, default=0.999)
        # aba augmentation
        parser.add_argument('--abd_augmentation', type = str2bool, default = True)
        # global memory config
        parser.add_argument('--global_memory_number', type = int, default = 8)
        parser.add_argument('--gaussain_mem', type = str2bool, default = False)
        parser.add_argument('--logits_resource', default='all', type=str, choices=['label_emb', 'feature', 'all'])
        parser.add_argument('--aba_ablation', type = str2bool, default = False)
        parser.add_argument('--abd_ablation', type = str2bool, default = False)
    ...
    def forward(self, x, label=None, get_rep=False, update_memory=True, use_local_mem=True, use_aba=True, use_global=True):
        ...
        h1 = self.encoder.forward_head(self.encoder.forward_features(x), pre_logits=True)
        # directly output
        if not self.local_mem_size and not self.abd_augmentation:
            logits = self.fc(nn.functional.relu(h1))
            return logits
        ori_bs = h1.shape[0]
        if self.model_config.aba_ablation:
            c1 = h1
        else:
        ...
        # (b) local memory augmentation
        if self.local_mem_size and use_local_mem and not self.model_config.aba_ablation:
            ...
            if label is not None:
                with torch.no_grad():
                    ...
                    if self.encoder_momentum:
                        ...
                        mem_feature = self.momen_encoder.forward_head(self.momen_encoder.forward_features(x), pre_logits=True)
            ...
            c1 = c1.unsqueeze(1) # bs, 1, d
            c1 = self.local_read_block(Q=c1, KV=c1) # bs, 1, d
            c1 = c1.squeeze(1) # bs, d
        
        # (c) attention between data points augmentation
        if self.abd_augmentation and use_aba and not self.model_config.abd_ablation:
            ...
            c1 = c1.unsqueeze(1) # bs, 1, d
            c1 = self.ABD_block(c1, KV=c1) # bs, 1, d
            c1 = c1.squeeze(1) # bs, d
        
        # predict with different features
        if self.logits_resource == 'label_emb' and not self.model_config.aba_ablation:
            label_feature = c1[:, self.dim_out:]
        elif self.logits_resource == 'feature' and not self.model_config.aba_ablation:
            label_feature = c1[:, :self.dim_out]
        else:
            label_feature = c1
