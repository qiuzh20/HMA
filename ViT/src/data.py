...
from torchvision.datasets import (CIFAR10, CIFAR100, DTD, STL10, FGVCAircraft,
                                  ...
                                  OxfordIIITPet, StanfordCars, Places365)
import torch
...
DATASET_DICT = {
    ...
    "places365": [
        partial(Places365, split="train", download=True),
        None,
        partial(Places365, split="test", download=True),
        365,
    ],
...
class DataModule(pl.LightningDataModule):
    def __init__(
        self,
        dataset: str = "cifar10",
        root: str = "/mnt/share/agiuser/qiuzihan/data",
    ...
    def prepare_data(self):
        if self.dataset != "custom":
            ...
            if self.val_dataset_fn is not None:
        ...
        if self.dataset == "custom":
            ...
            if stage == "fit":
                if self.val_dataset_fn is not None:
                    ...
                    print("without validation set, split from training set")
                    total_data = self.train_dataset_fn(self.root, transform=self.transforms_train, download=False)
                    if self.dataset == 'pets37':
                        ratio = 0.9
                    else:
                        ratio = 0.95
                    self.train_dataset, self.val_dataset = torch.utils.data.random_split(total_data, [int(len(total_data)*ratio),len(total_data)-int(len(total_data)*ratio)])
                    self.val_dataset.dataset.transform = self.transforms_test
