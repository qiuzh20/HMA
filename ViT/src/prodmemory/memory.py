...
class HashingMemory(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, params):
        ...
        self.size = params.mem_n_keys**2 if params.mem_product_quantization else params.mem_n_keys
        self.modulo_size = params.mem_modulo_size
        self.n_indices = self.size
        ...
        print("using product quantization:", params.mem_product_quantization)
        ...
        if len(params.mem_query_layer_sizes) > 0:
            ...
            l_sizes = [int(x) for x in filter(None, params.mem_query_layer_sizes.split(','))]
    ...
    def register_args(parser):
        ...
        parser.add_argument("--mem_implementation", type=str, default="flat", #"pq_fast",
        ...
        parser.add_argument("--mem_values_optimizer", type=str, default="adam_inverse_sqrt,lr=0.0010,warmup_updates=30000,beta1=0.9,beta2=0.999,weight_decay=0.01,eps=0.000001",#"adam,lr=0.001",
...
class HashingMemoryFlat(HashingMemory):
    ...
    def _get_indices(self, query, knn, keys):
        ...
        # scores2, indices2 = get_knn_faiss(keys2.float(), q2.float(), min(knn, self.n_keys), distance='dot_product')  # (bs, knn) ** 2
        # indices = get_knn_faiss(keys.float().contiguous(), query.float().contiguous(), knn, distance='dot_product')[1]
        # scores = (keys[indices] * query.unsqueeze(1)).sum(2)
...
class HashingMemoryProduct(HashingMemory):
    ...
    def _get_indices(self, query, knn, keys1, keys2):
        ...
        with torch.no_grad():
            ...
            scores1, indices1 = get_knn_faiss(keys1.float(), q1.float(), min(knn, self.n_keys), distance='dot_product')  # (bs, knn) ** 2
            scores2, indices2 = get_knn_faiss(keys2.float(), q2.float(), min(knn, self.n_keys), distance='dot_product')  # (bs, knn) ** 2
...
class HashingMemoryProductFast(HashingMemoryProduct):
    ...
    def _get_indices(self, query, knn, keys1, keys2):
        ...
        used_n_keys = min(knn, self.n_keys)
        
        # compute indices with associated scores
        scores1 = F.linear(q1, keys1, bias=None)                                                                      # (bs, n_keys ** 0.5)
        scores2 = F.linear(q2, keys2, bias=None)                                                                      # (bs, n_keys ** 0.5)
        scores1, indices1 = scores1.topk(used_n_keys, dim=1, largest=True, sorted=True)                                       # (bs, knn) ** 2
        scores2, indices2 = scores2.topk(used_n_keys, dim=1, largest=True, sorted=True)                                       # (bs, knn) ** 2
        ...
        # cartesian product on best candidate keys
        all_scores = (
            scores1.view(bs, used_n_keys, 1).expand(bs, used_n_keys, used_n_keys) +
            scores2.view(bs, 1, used_n_keys).expand(bs, used_n_keys, used_n_keys)
        ).view(bs, -1)                                                                                                # (bs, knn ** 2)
        all_indices = (
            indices1.view(bs, used_n_keys, 1).expand(bs, used_n_keys, used_n_keys) * n_keys +
            indices2.view(bs, 1, used_n_keys).expand(bs, used_n_keys, used_n_keys)
        ).view(bs, -1)                                                                                                # (bs, knn ** 2)
